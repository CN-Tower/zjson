...
[v] 支持错误的json解析及错误行号和错误类型提示；
[v] 支持python unicode集合的解析；
[v] 支持保存、删除和呈现格式化的历史；
[v] 支持格式化后的json以文件的形式下载；
[v] 支持中英文双语国际化；
[v] 支持线上json分享；
[v] 支持多种皮肤切换；
[v] 支持JSON转义、压缩；
[v] 支持JSON标准化析，把非标准JSON（如：js的object）转化为标准JSON格式；
[v] 支持JSON对比（有与原码对比、与历史比对和与新建代码对比三种模式）；
[v] 还有更多的功能、配置、特性等你去发现！
