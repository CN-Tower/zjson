...
import { ZJSON_SAVE_DIFFS } from '@/config'
import { DeleteOutlined } from '@ant-design/icons-vue'
import { formatDate } from '@/utils'

const emit = defineEmits(['select'])
const handleOpenHistory = inject('handleOpenHistory') as Func
const saveList = ref(JSON.parse(localStorage.getItem(ZJSON_SAVE_DIFFS) || '[]') as any[])
...
const handleDelItem = (i: number) => {
  ...
  localStorage.setItem(ZJSON_SAVE_DIFFS, JSON.stringify(saveList.value))
}
</script>
